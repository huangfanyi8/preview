...
static_assert(preview::derived_from<B, B> == true);      // same class: true
static_assert(preview::derived_from<int, int> == false); // same primitive type: false
static_assert(preview::derived_from<B, A> == true);      // public inheritance: true
static_assert(preview::derived_from<C, A> == false);     // private inheritance: false
